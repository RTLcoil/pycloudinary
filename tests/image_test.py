...
class ImageTest(unittest.TestCase):
    ...
    def test_build_url(self):
        ...
        self.assertEqual(
            ...
            '{url}/{id}'.format(**self.common_format))
    ...
    def test_image(self):
        ...
        self.assertEqual(
            self.image.image(),
            '<img src="{url}/{id}"/>'.format(**self.common_format))
    ...
    def test_scale(self):
        ...
        self.assertEqual(
            ...
            '<img height="100" '
            'src="{url}/c_scale,h_100,w_100/{id}" '
            'width="100"/>'.format(**self.common_format))
    ...
    def test_responsive_width(self):
        ...
        self.assertEqual(
            ...
            'data-src="{url}/c_limit,w_auto/{id}"/>'.format(
                             **self.common_format))
    ...
    def test_dpr_auto(self):
        ...
        self.assertEqual(
            ...
            '<img class="cld-hidpi" '
            'data-src="{url}/dpr_auto/{id}"/>'.format(**self.common_format))
    ...
    def test_client_hints_as_options(self):
        self.shared_client_hints(
            dpr="auto", cloud_name=self.cloud_name, width="auto",
            crop="scale", client_hints=True)
    ...
    def test_client_hints_as_false(self):
        ...
        tag = CloudinaryImage(self.full_public_id).image(
            ...
            cloud_name=self.cloud_name, client_hints=False)
        six.assertRegex(self, tag, '<img.*>')
        six.assertRegex(self, tag, 'class=["\']cld-responsive["\']')
        six.assertRegex(
            self, tag,
            'data-src=["\']{url}/c_scale,w_auto/{id}["\']'.format(**self.common_format))

    def test_width_auto_breakpoints(self):
        """supports auto width"""
        tag = CloudinaryImage(self.full_public_id).image(
            crop="scale", dpr="auto", cloud_name=self.cloud_name,
            width="auto:breakpoints", client_hints=True)
        six.assertRegex(
            self, tag,
            'src=["\']{url}/c_scale,dpr_auto,w_auto:breakpoints/{id}["\']'.format(**self.common_format))
